...
RV_ex = expon.ppf(YPlot, *exp_params)
...
bootstrap_rv_ex = np.zeros((num_bootstrap, len(Rp)))
# Bootstrap loop
for i in range(num_bootstrap):
    ...
    bootstrap_params_ex = expon.fit(bootstrap_sample)

    # Compute return values
    bootstrap_rv[i, :]    = genpareto.ppf(YPlot, *bootstrap_params)
    bootstrap_rv_ex[i, :] = expon.ppf(YPlot, *bootstrap_params_ex)
...
lower_ci_ex = np.percentile(bootstrap_rv_ex, 2.5, axis=0)
upper_ci_ex = np.percentile(bootstrap_rv_ex, 97.5, axis=0)
...
plt.plot(Rp, RV_ex, label='Exponential', color='green',linestyle = '-',
         linewidth=2)

# plt.fill_between(Rp, lower_ci, upper_ci, color='red', alpha=0.15,linestyle = '--',
#                   label='95% CI')

plt.fill_between(Rp, lower_ci_ex, upper_ci_ex, color='green', alpha=0.15,linestyle = '--',
                 label='95% CI')
...
Tp_values = Data.loc[POT_var.index, 'Tp'].to_numpy().flatten()
WS_values = Data.loc[POT_var.index, 'WindSpeed'].to_numpy().flatten()
WD_values = Data.loc[POT_var.index, 'WindDir'].to_numpy().flatten()
