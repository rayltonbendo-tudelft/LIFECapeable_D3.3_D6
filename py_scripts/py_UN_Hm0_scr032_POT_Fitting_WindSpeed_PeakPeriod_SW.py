...
# POT 
Threshold  = np.quantile(Data['Hm0'],0.9975)
DeclusTime = 12

POT_var = get_extremes(Data['Hm0'], "POT", threshold=Threshold, 
                                r=f"{DeclusTime}h")


Data = Data.loc[POT_var.index]
...
# Fit the curve
Tp_params, covariance = curve_fit(model, Hs, Tp, p0=[1, 1, 1])  
# Extract the optimal parameters
a_opt, b_opt, c_opt = Tp_params
...
Tp_fit = model(Hs_fit, a_opt, b_opt, c_opt)
...
ax.plot(Hs_fit, Tp_fit, label=f"Fit: y = {a_opt:.3f} * x^{b_opt:.3f} + {c_opt:.3f}"
        , color="red", linewidth=2)

ax.set_ylabel("Tp [s]")
ax.set_xlabel("Hm0 [m]")
ax.set_ylim([np.nanmin(Tp) - 1.0, np.nanmax(Tp) + 1.0])  
...
# Fit the curve
WS_params,Cov  = curve_fit(model, Hs, WindSpd, p0=[1, 1, 1])  # Initial guesses
a_opt, b_opt, c_opt = WS_params
...
plt.plot(Hs_fit, WindSpd_fit , label=f"Fit: y = {a_opt:.3f} * x^{b_opt:.3f} + {c_opt:.3f}", color="red")
