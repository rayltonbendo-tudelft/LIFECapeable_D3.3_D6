...
# %% Read data 
Data0 = pd.read_csv('Data/Data_Falsterbo10958_YSTAD.csv', 
                   parse_dates=['Time'])
...
ID_Dir  = (Data0.loc[Data0.index, 'Thetap'] >= 155) & (Data0.loc[Data0.index, 'Thetap'] < 270)
DataDir = Data0.loc[Data0.index[ID_Dir]].copy()

# %% POT 
Var        = DataDir['Hm0']
Threshold  = np.quantile(Var,0.9975)
DeclusTime = 12
...
# Fit Distributions
hs_Threshold  =  min(Var1)
params_hs = genpareto.fit(Var1 - hs_Threshold, floc=0, scale=hs_Threshold)  # GPD fit above threshold
params_hs = gpd_params_hs[:1] + (hs_Threshold,) + gpd_params_hs[2:]

params_wl = beta.fit(Var2) 
...
# Compute return values
RV_hs = genpareto.ppf(1 - 1 / (Rp1 * lambda1), *params_hs)
RV_wl = beta.ppf(1 - 1 / (Rp2 * lambda1), *params_wl)
...
ax11.plot(Rp1, RV_hs, '-k', linewidth=1.5, label='Generalized Pareto')
