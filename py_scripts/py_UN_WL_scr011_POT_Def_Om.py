...
for jj, thr in enumerate(Thr):
    ...
    for kk, dec_time in enumerate(DecTime):
        extremes = get_extremes(Data['SSH'], "POT", threshold=Data['SSH'].quantile(thr), 
                                r=f"{dec_time}d")
...
# Omnidirectional
Yplot = np.quantile(Data['SSH'], Thr) #np.round(Thr, 3)
...
# Mean residual life
VAR           = Data['SSH']
# MyThreshold   = 0.70 + MSL/100
MyThreshold   = np.quantile(VAR, 0.998)
min_threshold = 0.4 #np.quantile(VAR, 0.95)
max_threshold = 1.4 #np.quantile(VAR, 0.999) * 1.16
DTime         = 24 * 2
...
plot_threshold_stability(
    VAR,
    return_period=100,
    thresholds=np.linspace(0.5, 1.3, 20),
