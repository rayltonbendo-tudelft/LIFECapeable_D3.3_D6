...
# %% Reading data 
Data = pd.read_csv('Data/Falsterbo_01_South_Wave10958_WD_POT_SW_RH2000.csv', 
                   parse_dates=['Time'])
Wl  = Data['SSH']
Hs  = Data['Hm0']
...
cop_fam = []
# No need to rotate 
cop_fam.append(pv.BicopFamily.indep) 
...
# Save results in csv
# aic_bic_table.to_csv("Data/CopulaSelection_SW_aic_bic_results.csv", index=False)
# %%  ---- FITTING OF THE BEST COPULA ----     
#-- Fitting margins 
Threshold =  min(Hs)
params_hs = genpareto.fit(Hs - Threshold, floc=0, scale=Threshold) 
params_hs = params_hs[:1] + (Threshold,) + params_hs[2:]

params_wl   = beta.fit(Wl) 


#-- Fitting the best copula family 
copFLOOD_T = pv.Bicop( pv.BicopFamily.indep, rotation = 0)
...
FLOOD_Tsim = copFLOOD_T.simulate(n = 10**4)
FLOOD_Tsim[:,0]  = FLOOD_Tsim[:,0]
FLOOD_Tsim[:,1]  = FLOOD_Tsim[:, 1]

# Apply the transformation to the simulated data

FLOODfp_Tsim =  genpareto.ppf(FLOOD_Tsim[:,0], *params_hs)
FLOODfv_Tsim =  beta.ppf(FLOOD_Tsim[:,1], *params_wl)
...
def return_period(probability):
    return (1 / probability) / lambda1 if probability > 0 else float("inf")

def scenario_probability(u1, u2, copula, scenario):
    copula_cdf = copula.cdf(np.array([[u1, u2]]))
    if scenario == "or":
        return 1 - copula_cdf
    elif scenario == "and":
        return 1 - u1 - u2 + copula_cdf
...
WL_thresholds = np.linspace(min(Wl), 1.5, 1000)
Hs_thresholds = np.linspace(min(Hs), 7.0, 1000)

# Generate grid of thresholds
X, Y = np.meshgrid(WL_thresholds, Hs_thresholds)
u1 = beta.cdf(X, *params_wl) 
u2 = genpareto.cdf(Y, *params_hs)
...
# Return periods
Z_or = np.where(prob_or > 0, (1 / prob_or) / lambda1, float("inf"))
Z_and = np.where(prob_and > 0, (1 / prob_and) / lambda1, float("inf"))
...
ax11.set_xlim([-0.75, 1.5])
ax11.set_ylim([2.6, 6.5])
...
ax12.set_xlim([-0.75, 1.5])
ax12.set_ylim([2.6, 6.5])
...
# Uncomment to save
df_and.to_csv("Data/Falsterbo_07_South_BivRP_WD_AND_SW_RH2000.csv", index=False)
df_or.to_csv("Data/Falsterbo_08_South_BivRP_WD_OR_SW_RH2000.csv", index=False)
